...
import kg.bitruby.authservice.common.config.granttypes.email_password.EmailPasswordAuthenticationConverter;
import kg.bitruby.authservice.common.config.granttypes.email_password.EmailPasswordAuthenticationProvider;
import kg.bitruby.authservice.common.config.granttypes.email_password.EmailPasswordAuthenticationToken;
import kg.bitruby.authservice.common.config.granttypes.phone_password.PhonePasswordAuthenticationConverter;
import kg.bitruby.authservice.common.config.granttypes.phone_password.PhonePasswordAuthenticationProvider;
import kg.bitruby.authservice.common.config.granttypes.phone_password.PhonePasswordAuthenticationToken;
import kg.bitruby.authservice.common.config.model.CustomPasswordUser;
import kg.bitruby.authservice.core.UsersService;
...
import static kg.bitruby.authservice.api.model.GrantType.EMAIL_PASSWORD;
...
public class SecurityConfig {
  ...
  private final UsersService userInfoService;
  ...
  @Bean
  public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer(UsersService userInfoService) {
    return context -> {
  ...
  private UserEntity getUserInfo(JwtEncodingContext context, UsersService userInfoService) {
  ...
  private UserEntity getUserInfoFromRefreshToken(OAuth2Authorization authorization, UsersService userInfoService) {
  ...
  private String determineUserLevel(UserEntity userInfo) {
    ...
    switch (userInfo.getAccountStatus()) {
      case ACCOUNT_LOCK -> {
        ...
        return "0";
      } case BYBIT_ACCOUNT_NOT_CREATED -> {
        return "1";
      } case OK -> {
        return "2";
      } case CREDENTIAL_EXPIRED -> throw new OAuth2AuthenticationException("Credential expired");
      case REGISTRATION_STAGE -> throw new OAuth2AuthenticationException("Account not enable");
      default -> throw new OAuth2AuthenticationException("Authorization error. Illegal account status");
    }
  }
